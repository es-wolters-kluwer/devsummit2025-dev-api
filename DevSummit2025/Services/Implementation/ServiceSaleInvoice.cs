...
namespace DevSummit2025.Services.Implementation
{
    public class ServiceSaleInvoice(IConfiguration configuration, IServiceLogin serviceLogin, IServiceCustomer serviceCustomer, IServiceProduct serviceProduct) : IServiceSaleInvoice
    {
        ...
        {
            var invoice = await GetNew();       
            
            var lineManual = await GetNewLine();            
            lineManual.description = "Producto teste";
            lineManual.price = 15;

            var lineProduct = await GetNewLine(); 
            var productList = await serviceProduct.GetAll();
            lineProduct.productId = productList[1].id;

            invoice.lines = new object[] { lineManual, lineProduct };
            
            var customerList = await serviceCustomer.GetAll();
